!
! Script para construir o modulo Thompson09 proveniente do MPAS para o KiD
! Marilia Gregorio - novembro 2023
...
!     USE module_domain, ONLY : HISTORY_ALARM, Is_alarm_tstep
      ...
      !logical, parameter, private:: iiwarm = .false. - vem do namelists
...
!.. Somente no MPAS... pq o MPAS contém microfisica da Aerosol
...
!.. Thompson et al (2008). Coefficient fv is zero for graupel/ice.
      ...
      REAL, PARAMETER, PRIVATE:: av_c = 0.316946E8    ! MPAS
      REAL, PARAMETER, PRIVATE:: bv_c = 2.0           ! MPAS
...
!..Water vapor and air gas constants at constant pressure
      ...
      REAL, PARAMETER, PRIVATE:: R_uni = 8.314                           ! J (mol K)-1 - somente no MPAS

      DOUBLE PRECISION, PARAMETER, PRIVATE:: k_b = 1.38065E-23           ! Boltzmann constant [J/K] - somente no MPAS
      DOUBLE PRECISION, PARAMETER, PRIVATE:: M_w = 18.01528E-3           ! molecular mass of water [kg/mol] - somente no MPAS
      DOUBLE PRECISION, PARAMETER, PRIVATE:: M_a = 28.96E-3              ! molecular mass of air [kg/mol] - somente no MPAS
      DOUBLE PRECISION, PARAMETER, PRIVATE:: N_avo = 6.022E23            ! Avogadro number [1/mol] - somente no MPAS
      DOUBLE PRECISION, PARAMETER, PRIVATE:: ma_w = M_w / N_avo          ! mass of water molecule [kg] - somente no MPAS
      REAL, PARAMETER, PRIVATE:: ar_volume = 4./3.*PI*(2.5e-6)**3        ! assume radius of 0.025 micrometer, 2.5e-6 cm - somente no MPAS
...
!..Lookup table dimensions
      ...
      INTEGER, PARAMETER, PRIVATE:: ntb_arc = 7 !- somente no MPAS n° total de categorias de particulas para aglomerados de nuvens de tipo cristais de gelo
      INTEGER, PARAMETER, PRIVATE:: ntb_arw = 9 !- somente no MPAS n° total de categorias de particulas para aglomerados de nuvens de tipo agulhas de gelo
      INTEGER, PARAMETER, PRIVATE:: ntb_art = 7 !- somente no MPAS n° total de categorias de particulas para aglomerados de nuvens de tipo particulas de neve
      INTEGER, PARAMETER, PRIVATE:: ntb_arr = 5 !- somente no MPAS n° total de categorias de particulas para aglomerados de nuvens de tipo gotas de chuva
      INTEGER, PARAMETER, PRIVATE:: ntb_ark = 4 !- somente no MPAS n° total de categorias de particulas para aglomerados de nuvens de tipo granizo
      INTEGER, PARAMETER, PRIVATE:: ntb_IN = 55 !- somente no MPAS n° total de categorias de particulas para particulas de gelo grande
      INTEGER, PRIVATE:: niIN2 !- somente no MPAS - possivelmente relacionado ao n° de categorias para particulas de gelo grande
      ...
      DOUBLE PRECISION, DIMENSION(nbc):: t_Nc !- somente no MPAS 
...
!.. allocatable (2009Jun12, J. Michalakes).
      ...
      REAL (KIND=R8SIZE), ALLOCATABLE, DIMENSION(:,:,:)::               &
                tpc_wev, tnc_wev
      REAL (KIND=R4SIZE), ALLOCATABLE, DIMENSION(:,:,:,:,:):: tnccn_act !ficar atenta aqui
...
!..Constants for helping find lookup table indexes.
      ...
      niIN2 = NINT(ALOG10(Nt_IN(1)))     !Somente no MAPS...NOVA TABELA
...
!..Create bins of cloud droplet number concentration (1 to 3000 per cc). - Somente no MPAS
      xDx(1) = 1.0d0
      xDx(nbc+1) = 3000.0d0
      do n = 2, nbc
...
!KiD      CALL wrf_debug(200, '  creating qc collision eff tables')
     ...
     !..Drop evaporation.
!     call physics_message('--- creating rain evap table') - MPAS
         call table_dropEvap    
...
!     endif
      ...
      do k = kts, kte
            ...
            nr1d(k) = nr(i,k,j)
      enddo
      if (is_aerosol_aware) then  !parte de aerosol que só tem no MPAS
            do k = kts, kte
               nc1d(k) = nc(i,k,j)
               nwfa1d(k) = nwfa(i,k,j)
               nifa1d(k) = nifa(i,k,j)
...
! SOMENTE PARA AEROSOL QUE ESTÁ NO MPAS
...
!
        subroutine mp_thompson (qv1d, qc1d, qi1d, qr1d, qs1d, qg1d, ni1d, &
            nr1d, nc1d, nwfa1d, nifa1d, t1d, p1d, w1d, dzq, &
            pptrain, pptsnow, pptgraul, pptice, & !            pptrain_level,                                &
#ifdef WRF_CHEM
            rainprod, evapprod, &
#endif
            kts, kte, dt, ii, jj)
...
!INTEGER, DIMENSION(5):: ksed1 - Marilia 11/01/2024
...
!+---+-----------------------------------------------------------------+
      ...
      do k = kts, kte
            temp(k) = t1d(k) !Perfil vertical de temperatura.
            qv(k) = MAX(1.E-10, qv1d(k)) !Perfil vertical de vapor d'água, com um valor mínimo 1.e-10
            pres(k) = p1d(k) !perfil vertical de pressão
            rho(k) = 0.622*pres(k)/(R*temp(k)*(qv(k)+0.622)) !perfil vertical da densidade do ar
            nwfa(k) = MAX(11.1E6, MIN(9999.E6, nwfa1d(k)*rho(k)))  !MPAS - numero de particulas de agua na fase liquida
            nifa(k) = MAX(naIN1*0.01, MIN(9999.E6, nifa1d(k)*rho(k)))   !MPAS - numero de particulas de gelo

            if (qc1d(k) .gt. R1) then ! qc1d -> concentração de nuvens em forma liquida
                  no_micro = .false.
                  rc(k) = qc1d(k)*rho(k)
                  ...
                  qc1d(k) = 0.0
                  nc1d(k) = 0.0 !numero de goticulas de nuvens liquidas na vertical
                  rc(k) = R1 !raio medio das goticulas
                  nc(k) = 2.  !numero de goticulas de nuvens em forma liquida no ponto vertical k.
                              !Se houver nuvens líquidas, esse número é definido de acordo com a microfísica do modelo. 
                  L_qc(k) = .false. !indica que inicialmente não há nuvens liquidas neste ponto.
            endif

            if (qi1d(k) .gt. R1) then !qi1d -> concentração de nuvens de gelo
                  no_micro = .false.
                  ri(k) = qi1d(k)*rho(k)
                  ...
                  qi1d(k) = 0.0 
                  ni1d(k) = 0.0 !numero de goticulas de nuvens de gelo na vertical
                  ri(k) = R1 !raio medio das goticulas
                  ni(k) = R2 !numero de goticulas de nuvens em forma de gelo no ponto vertical k.
                  L_qi(k) = .false.
            endif

            if (qr1d(k) .gt. R1) then !concentração de chuva
                  no_micro = .false.
                  rr(k) = qr1d(k)*rho(k)
            ...
            elseif (mvd_r(k) .lt. D0r*0.75) then
               ...
               nr(k) = crg(2)*org3*rr(k)*lamr**bm_r / am_r
                  ...
                  qr1d(k) = 0.0
                  nr1d(k) = 0.0 !numero de goticulas de chuva na vertical
                  rr(k) = R1 !raio medio das goticulas
                  nr(k) = R2 !numero de goticulas de nuvens em forma de chuva no ponto vertical k.
                  L_qr(k) = .false.
            endif
            if (qs1d(k) .gt. R1) then !concetração de neve
                  no_micro = .false.
                  rs(k) = qs1d(k)*rho(k)
                  L_qs(k) = .true.
            else
                  qs1d(k) = 0.0
                  rs(k) = R1 !raio medio das goticulas
                  L_qs(k) = .false.
            endif
            if (qg1d(k) .gt. R1) then !concetração de graupel
                  no_micro = .false.
                  rg(k) = qg1d(k)*rho(k) 
                  L_qg(k) = .true.
            else
                  qg1d(k) = 0.0
                  rg(k) = R1 !raio medio das goticulas
                  L_qg(k) = .false.
            endif
      enddo
...
!.. esponsável por calcular diversas propriedades termodinâmicas e 
!.. características físicas do ar em diferentes níveis.
!+---+-----------------------------------------------------------------+
      do k = kts, kte
            tempc = temp(k) - 273.15 !Converte a temperatura em Kelvin para graus Celsius
            rhof(k) = SQRT(RHO_NOT/rho(k)) !calcula a raiz da razão entre a densidade de referencia e a densidade do ar na vertical
            rhof2(k) = SQRT(rhof(k))
            qvs(k) = rslf(pres(k), temp(k)) !calculo da saturação especifica de vapor na vertical
            delQvs(k) = MAX(0.0, rslf(pres(k), 273.15)-qv(k)) !calculo da diferença entre a a saturação especifica 
                                                               !de vapor a o?C e a umidade especifica atual no ponto k
            if (tempc .le. 0.0) then
             qvsi(k) = rsif(pres(k), temp(k))
            else
             qvsi(k) = qvs(k)
            endif
            satw(k) = qv(k)/qvs(k) !calculo da razão de mistura pela saturação do vapor 
            sati(k) = qv(k)/qvsi(k) !calculo da razão de mistura pela saturaçaõ de vapor de gelo
            ssatw(k) = satw(k) - 1. 
            ssati(k) = sati(k) - 1.
            if (abs(ssatw(k)).lt. eps) ssatw(k) = 0.0
            if (abs(ssati(k)).lt. eps) ssati(k) = 0.0
            if (no_micro .and. ssati(k).gt. 0.0) no_micro = .false.
            diffu(k) = 2.11E-5*(temp(k)/273.15)**1.94 * (101325./pres(k)) !calculo da difusidade molecular
                                                                           !utiliza a formula que considerada temp e pressao
            if (tempc .ge. 0.0) then
               visco(k) = (1.718+0.0049*tempc)*1.0E-5 !viscosidade do ar
            else
               visco(k) = (1.718+0.0049*tempc-1.2E-5*tempc*tempc)*1.0E-5
            endif
            ocp(k) = 1./(Cp*(1.+0.887*qv(k))) !capacidade calorifica especifica do ar quando a pressao é constante
            vsc2(k) = SQRT(rho(k)/visco(k))
            lvap(k) = lvap0 + (2106.0 - 4218.0)*tempc !calor latente de vaporização de agua com base na temperatua
            tcond(k) = (5.69 + 0.0168*tempc)*1.0E-5 * 418.936 !condutividade termica do ar
      enddo
...
!+---+-----------------------------------------------------------------+
!..Calculate y-intercept, slope values for graupel. - IGUAL AO DO MPAS
!+---+-----------------------------------------------------------------+
      N0_min = gonv_max
      k_0 = kts
      do k = kte, kts, -1
...
!..Cloud droplet number lookup table index. - SOMENTE NO MPAS
...
!..Snow collecting cloud water.  In CE, assume Dc<<Ds and vtc=~0.
      if (L_qc(k) .and. mvd_c(k).gt. D0c) then
       ...
       if (xDs .gt. D0s) then
        ...
        pnc_scw(k) = rhof(k)*t1_qs_qc*Ef_sw*nc(k)*smoe(k)                ! Qc2M - MPAS
        pnc_scw(k) = MIN(DBLE(nc(k)*odts), pnc_scw(k))                   ! MPAS
      endif
      ...
      if (rg(k).ge. r_g(1) .and. mvd_c(k).gt. D0c) then
            ...
            if (xDg.gt. D0g) then
             ...
             pnc_gcw(k) = rhof(k)*t1_qg_qc*Ef_gw*nc(k)*N0_g(k)           &
                           *ilamg(k)**cge(9)                                 ! Qc2M - MPAS
             pnc_gcw(k) = MIN(DBLE(nc(k)*odts), pnc_gcw(k))
            endif
           endif
      endif

!..Snow and graupel collecting aerosols, wet scavenging. - SOMENTE NO MPAS
      if (rs(k) .gt. r_s(1)) then
            xDs = smoc(k) / smob(k)
...
!.. results in lookup table.
       ...
       !..Put in explicit drop break-up due to collisions.
                   pnr_rcg(k) = -5.*tnr_gacr(idx_g1,idx_g,idx_r1,idx_r)         ! RAIN2M - MPAS
                  endif
                 endif
                endif
...
!.. Implemented by T. Eidhammer and G. Thompson 2012Dec18 - MPAS
!+---+-----------------------------------------------------------------+
...
!..Rain tendency
!    qrten(k): Representa a concentração total de água na forma de gotículas de chuva no nível vertical k. 
!	            A expressão à direita indica como essa concentração é atualizada com base em várias contribuições e subtrações.
!    prr_wau(k): Representa a taxa de produção de chuva devido à autoagregação de gotículas de água.
!    prr_rcw(k): Representa a taxa de produção de chuva devido à colisão e coalescência de gotículas de água.
!    prr_sml(k): Representa a taxa de produção de chuva devido a algum processo relacionado ao tamanho das gotículas.
!    prr_gml(k): Representa a taxa de produção de chuva devido ao processo de autoagregação de gotículas de água de tamanho grande.
!    prr_rcs(k): Representa a taxa de produção de chuva devido à colisão e coalescência de gotículas de chuva e nuvens.
!    prr_rcg(k): Representa a taxa de produção de chuva devido à colisão e coalescência de gotículas de chuva e granizo.
!    prg_rfz(k): Representa a taxa de produção de granizo devido ao congelamento de gotículas de chuva.
!    pri_rfz(k): Representa a taxa de produção de granizo devido ao congelamento de gotículas de gelo.
!    prr_rci(k): Representa a taxa de produção de chuva devido à colisão e coalescência de gotículas de chuva e gelo.
!    orho: Representa a densidade ou densidade relativa. 
...
!.. from lookup table.
!.. Aerossois CNN => particulas microscopicas presentes na atmosfera
!.. como poeira, poluentes ou outros compostos. CNN (núcleo de condensaçao
!.. de nuvens) são particulas especificas que tem a capacidade de atuar
!.. como nucleos de condensação para a formação de goticulas de nuvem.
!.. Higroscopicidade: refere-se a propriedade de absorver água do ambiente.
!.. Essa parte só tem no MPAS.
!..  **`tempc` (Temperatura em graus Celsius):** Calculada subtraindo 273.15 da temperatura (`temp(k)`).
!.. - **`otemp` (Recíproco da temperatura):** Calculado como `1./temp(k)`.
!.. - **`rvs` (Densidade de vapor de saturação):** Calculado multiplicando a densidade do ar (`rho(k)`) pela densidade de vapor de saturação (`qvs(k)`).
!.. - **`rvs_p` e `rvs_pp` (Derivadas de `rvs` em relação à temperatura):** Usados em cálculos subsequentes relacionados à evaporação.
!.. - **`gamsc` (Parâmetro de troca de calor):** Representa uma taxa de troca de calor relacionada à evaporação.
!.. - **`alphsc` (Parâmetro adimensional):** Representa um coeficiente adimensional relacionado à evaporação.
!.. - **`xsat` (Saturação de água):** Valor da saturação de água (`ssatw(k)`).
!.. - **`t1_evap` (Fator na equação de evaporação):** Calculado com base nos parâmetros previamente definidos.
!.. - **`Dc_star` (Diâmetro crítico para evaporação):** Calculado como a raiz quadrada de uma expressão envolvendo o fator `t1_evap`.
!.. - **`idx_d`, `idx_n`, e `idx_c` (Índices para consulta de tabelas):** Usados para indexar tabelas de evaporação.
!.. - **`prw_vcd` (Quantidade de água líquida formada):** Atualizada considerando a evaporação de água de nuvem. O valor é o máximo entre zero e uma fração de água de nuvem evaporada.
!.. - **`pnc_wcd` (Alteração na concentração de nuvens):** Atualizada considerando a evaporação da concentração de nuvens. O valor é o máximo entre zero e uma fração da concentração de nuvens evaporada.
!.. - **`xnc` (Número de nuvens a serem formadas):** Calculado usando a função `activ_ncloud` com base na temperatura (`temp`), na taxa de queda de precipitação (`w1d`), e em um fator de nucleação de nuvens (`nwfa`).                          
...
!..TESTE: G. Thompson  10 de Maio de 2013
!..Reduzir a evaporação da chuva nos mesmos lugares onde ocorre a fusão de graupel.
!..Justificativa: a fusão simultânea de graupel em regiões subsaturadas não derrete tão rapidamente
!..porque a temperatura da partícula permanece em 0ºC. Além disso, há pouca perda de água do graupel,
!..então é provável que o graupel revestido de água evapore muito mais lentamente do que
!..se a água fosse imediatamente removida.
...
!+---+-----------------------------------------------------------------+
!.. Calculos relacionados a altura acima do solo e a concentração
!.. de gotas de chuva. Somente no MPAS
!+---+-----------------------------------------------------------------+
! COMENTEI AQUI - MARILIA 11/01/2024
!   hgt_agl = 0.
!   do k = kts, kte-1
...
!+---+-----------------------------------------------------------------+
!.. esse trecho de código modela a sedimentação de gotículas de nuvem, 
!.. calculando as taxas de sedimentação, acumulando mudanças cumulativas 
!.. e atualizando as concentrações de massa e numéricas à medida que as 
!.. partículas se movem verticalmente na atmosfera.
!.. - `odzq` é o inverso do tamanho do intervalo vertical (`dzq`) no nível `k`.
!..   - `orho` é o inverso da densidade do ar no nível `k`.
!..   - `qcten` e `ncten` são acumuladores que representam as mudanças cumulativas nas variáveis de sedimentação.
!..   - As concentrações de massa (`rc`) e numéricas (`nc`) são atualizadas com base nas mudanças nas taxas de sedimentação, multiplicadas pelo intervalo vertical e pela densidade do ar.
!..   - `MAX(R1, ...)` garante que a concentração de massa (`rc`) não seja menor que um valor mínimo (`R1`).
!..   - `MAX(10., ...)` impõe um limite inferior para a concentração numérica (`nc`), garantindo que não seja inferior a 10.
...
!+---+-----------------------------------------------------------------+     
!.. KID
!..    vtik(k) e vtnik(k): Essas são as velocidades terminais para partículas 
!.. de gelo nas posições k. A velocidade terminal é a velocidade constante que 
!.. uma partícula atinge quando a força de arrasto do fluido é equilibrada pela
!.. força gravitacional. É calculada com base em características da partícula 
!.. e das condições atmosféricas locais.
!.. 
!..     ri(k): Esta é a raio da partícula de gelo na posição k. O raio é uma 
!.. medida fundamental que descreve o tamanho da partícula.
!.. 
!..     ni(k): Essa é a concentração de partículas de gelo na posição k. 
!.. Representa a quantidade de partículas por unidade de volume na atmosfera.
!.. 
!.. Portanto, sed_i(k) e sed_n(k) representam as taxas de sedimentação das 
!.. partículas de gelo nas posições k com base em suas velocidades terminais, 
!.. raios e concentrações. Essas taxas são usadas posteriormente no código para 
!.. atualizar variáveis que representam a evolução temporal dessas partículas, 
!.. levando em consideração a sedimentação. 
...
!.. KID
!+---+-----------------------------------------------------------------+  
!.. Esse trecho de código modela a sedimentação de neve na atmosfera, 
!.. atualizando as concentrações de massa de neve e as variáveis de 
!.. sedimentação ao longo de subpassos de tempo.
!.. 
!..    sed_s(k): Taxa de sedimentação de neve no nível vertical k. É calculada 
!..multiplicando a velocidade terminal (vtsk(k)) pela concentração de massa de 
!..neve no mesmo nível (rs(k)).
!..
!..    vtsk(k): Velocidade terminal de queda de neve no nível vertical k. Este 
!..valor é utilizado para calcular a taxa de sedimentação.
!..
!..    rs(k): Concentração de massa de neve no nível vertical k. Representa a 
!..quantidade de massa de neve por unidade de volume nesse nível.
!..
!.. Essas variáveis estão envolvidas no cálculo das taxas de sedimentação de neve, 
!.. sendo parte do modelo de microfísica atmosférica que simula os processos 
!.. relacionados às partículas de neve na atmosfera. O cálculo da taxa de sedimentação 
!.. leva em consideração a velocidade terminal das partículas e sua concentração de 
!.. massa em cada nível vertical da atmosfera.
...
!.. 
!.. Este trecho de código modela a sedimentação de graupel na atmosfera
!.. ao longo de vários subpassos de tempo, considerando a velocidade 
!.. terminal das partículas e sua concentração de massa em cada nível vertical.
!..    sed_g(k):
!..        É um array que armazena a taxa de sedimentação de graupel no nível vertical k.
!..
!..    vtgk(k):
!..        Representa a velocidade terminal de queda do graupel no nível vertical k.
!..
!..    rg(k):
!..        Representa a concentração de massa de graupel no nível vertical k.
!..
!..    sed_g(k) = vtgk(k)*rg(k):
!..       A cada iteração do loop, o valor de sed_g(k) é calculado multiplicando a 
!.. velocidade terminal vtgk(k) pela concentração de massa rg(k) no nível vertical 
!.. correspondente k e armazenando o resultado no array sed_g para o índice k.
!..
!.. Em resumo, esse trecho do código está calculando e armazenando a taxa de sedimentação
!.. de graupel em diferentes níveis verticais, levando em consideração a velocidade 
!.. terminal e a concentração de massa do graupel em cada nível.
...
!..    t1d(k): Atualização da temperatura em uma determinada camada.
!..    qv1d(k): Atualização da quantidade de vapor d'água em uma camada.
!..    qc1d(k): Atualização da quantidade de nuvens condensadas em uma camada.
!..    nc1d(k): Atualização da concentração de gotículas de nuvem em uma camada.
!..    nwfa1d(k): Atualização da concentração de partículas de gelo em uma camada.
!..    nifa1d(k): Atualização da concentração de partículas de gelo finas em uma camada.
!..    qi1d(k): Atualização da quantidade de gelo em uma camada.
!..    ni1d(k): Atualização da concentração de partículas de gelo em uma camada.
!..    qr1d(k): Atualização da quantidade de chuva em uma camada.
!..    nr1d(k): Atualização da concentração de gotas de chuva em uma camada.
!..    qs1d(k): Atualização da quantidade de neve em uma camada.
!..    qg1d(k): Atualização da quantidade de granizo em uma camada.
!+---+-----------------------------------------------------------------+
      do k = kts, kte
...
!..Freeze water (smallest drops become cloud ice, otherwise graupel).
       ...
       !         print*, ' Freezing water for temp = ', -k
                ...
                do j = 1, ntb_r1
                   do i = 1, ntb_r
                      ...
                      do n2 = nbr, 1, -1
                         ...
                         !if ((sum1+sum2).ge.r_r(i)) EXIT - nao tem no Kid - Marilia
                      enddo
                      tpi_qrfz(i,j,k) = sum1
                ...
                !do j = 1, nbc! original MPAS
                ! nu_c = MIN(15, NINT(1000.E6/t_Nc(j)) + 2) - original MPAS
                !coloquei t_Nc(1)
                nu_c = MIN(15, NINT(1000.E6/t_Nc(1)) + 2)
                do i = 1, ntb_c
                  lamc = (t_Nc(1)*am_r* ccg(2,nu_c) * ocg1(nu_c) / r_c(i))**obmr
                  N0_c = t_Nc(1)*ocg1(nu_c) * lamc**cce(1,nu_c)
                  sum1 = 0.0d0
                  sumn2 = 0.0d0
                  do n = nbc, 1, -1
                     vol = massc(n)*orho_w
                     ...
                     sumn2 = MIN(t_Nc(1), sumn2 + prob*N_c(n))
...
!+---+-----------------------------------------------------------------+
!+---+-----------------------------------------------------------------+
END MODULE module_mp_thompson09n
!+---+-----------------------------------------------------------------+
